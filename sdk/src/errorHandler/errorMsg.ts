export const errorMsg: any = {
  "429":
    "Rate limit exceeded for the given bundler api key. Please contact bundler team for increasing bandwidth.", // Rate limit quota execeeded
  ...
  "-32504":
    "This paymaster is not whitelisted on current bundler, contact bundler team to whitelist", // transaction rejected because paymaster (or signature aggregator) is throttled/banned
  "-32505":
    "Factory or Wallet or Paymaster not staked or unstake-delay is too low. Try with another entity", // transaction rejected because some entity (or signature aggregator) stake or unstake-delay is too low
  "-32506":
  ...
  "400":
    "Either the bundler url is unreachable or the api key rate limit has reached. Please contact support for more info", // Bundler using ethers package returning SERVER_ERROR
  "404":
    "The request sent has reached timeout. Check your internet access or the bundler url if using etherspot bundler, the rate limit might be reached Please contact support for more info", // ethers package
  "-429":
    "Rate limit exceeded for the given bundler api key. Please contact bundler team for increasing bandwidth.", // Rate limit quota execeeded
};

export const entryPointErrorMsg: any = {
  "AA10 sender already constructed":
    "The sender was already created, so the initCode does not need to be run. This error may occur if you attempt to create an account multiple times",
  ...
  "AA21 didn't pay prefund":
    "The sender did not have enough to prefund the EntryPoint for the user operation. If you are using a paymaster, the paymasterAndData field is likely not set. If you aren't using a paymaster, the address of the sender does not have enough gas token. After the user operation is executed, the remainder of the prefund is credited back to the sender",
  "AA22 expired or not due":
    "The signature is not valid because it is outside of the specified time range",
  "AA23 reverted (or OOG)":
    "The sender does not have sufficient native tokens to cover the User Operation's gas costs. If you intended to use a Paymaster for sponsorship, ensure that the paymasterAndData field of the user operation is correctly set to enable proper handling of gas fees",
  ...
  "AA31 paymaster deposit too low":
    "The paymaster is out of funds. More gas tokens must be deposited into the EntryPoint for the paymaster. This is usually done by calling the paymaster contract's deposit function. If you are using a paymaster service, contact them immediately",
  "AA32 paymaster expired or not due":
    "The paymaster's signature is not valid because it is outside of the specified time range",
  "AA33 reverted (or OOG)":
    "The paymaster validation was rejected or ran out of gas. 'OOG' is an abbreviation for Out-Of-Gas. First check the paymaster's signature in paymasterAndData. If the signature is correct, the verificationGasLimit may be too low",
