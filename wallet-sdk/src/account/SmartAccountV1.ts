...
export class SmartAccountV1 extends BaseSmartAccount {
   ...
   */
  accountContract?: SmartAccount_v100;

  factory?: SmartAccountFactory_v100;
  ...
  public static async create(biconomySmartAccountConfig: BiconomySmartAccountV2Config): Promise<SmartAccountV1> {
    ...
    const defaultFallbackHandlerAddress =
      instance.factoryAddress === DEFAULT_SMART_WALLET_FACTORY_ADDRESS
        ? DEFAULT_FALLBACK_HANDLER_ADDRESS
    ...
    instance.implementationAddress = biconomySmartAccountConfig.implementationAddress ?? SMART_WALLET_IMPLEMENTATION_ADDRESSES_BY_VERSION.V2_0_0;
  ...
  async _getAccountContract(): Promise<SmartAccount_v100> {
    if (this.accountContract == null) {
      this.accountContract = SmartAccount_v100__factory.connect(await this.getAccountAddress(), this.provider);
    }
  ...
  private async getCounterFactualAddressV2(params?: CounterFactualAddressParam): Promise<string> {
    if (this.factory == null) {
      if (this.factoryAddress != null && this.factoryAddress !== "") {
        this.factory = SmartAccountFactory_v100__factory.connect(this.factoryAddress, this.provider);
      } else {
    ...
    try {
      const initCalldata = SmartAccount_v100__factory.createInterface().encodeFunctionData("init", [
