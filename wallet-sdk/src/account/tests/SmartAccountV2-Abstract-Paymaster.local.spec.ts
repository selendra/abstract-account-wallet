...
import { SmartAccountV1 } from "../SmartAccountV1";
import { ChainId } from "../../core-types";
import { ECDSAOwnershipValidationModule } from "../../modules";
import { BaseValidationModule } from "../../modules";
import { ECDSAOwnershipRegistryModule_v100 } from "../../common";
import { BiconomyPaymaster } from "../../paymaster";

const provider = new ethers.providers.JsonRpcProvider("http://127.0.0.1:8545");
const signer = provider.getSigner();

describe("SmartAccountV1 Paymaster Abstraction", () => {
  let owner: Wallet;
  let factoryOwner: Wallet;
  let account: SmartAccountV1;
  let entryPoint: EntryPoint;
  let accountFactory: SmartAccountFactory_v100;
  ...
  beforeAll(async () => {
    ...
    const accountImpl: SmartAccount_v100 = await new SmartAccount_v100__factory(signer).deploy(entryPoint.address);

    accountFactory = await new SmartAccountFactory_v100__factory(signer).deploy(accountImpl.address, await factoryOwner.getAddress());
  ...
  it("Create a smart account with paymaster through api key", async () => {
    ...
    account = await SmartAccountV1.create({
      chainId: ChainId.SELENDRA_TESTNET,
  ...
  it("Create a smart account with paymaster by creating instance", async () => {
    ...
    account = await SmartAccountV1.create({
      chainId: ChainId.SELENDRA_MAINNET,
      rpcUrl: "http://127.0.0.1:8545",
      entryPointAddress: entryPoint.address,
      factoryAddress: accountFactory.address,
      defaultFallbackHandler: await accountFactory.minimalHandler(),
