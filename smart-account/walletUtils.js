...
const makeKeyList = (
  ...
  let i;
  for (i = 0; i < num; i++) {
    const mnemonicInstance = ethers.Mnemonic.fromPhrase(mn);
    accounts.push(ethers.HDNodeWallet.fromMnemonic(mnemonicInstance, path + i).privateKey);
  }
  return accounts;
};
...
const makeSignerList = (
  ...
  for (i = 0; i < num; i++) {
    const mnemonicInstance = ethers.Mnemonic.fromPhrase(mn);
    accounts.push(ethers.HDNodeWallet.fromMnemonic(mnemonicInstance, path + i));
  }
  return accounts;
};
