...
import { BitrielWalletAPI, HttpRpcClient, VerifyingPaymasterAPI } from './base';
 ...
 * bitriel-Sdk
 *
 * @category bitriel-Sdk
 */
export class BitrielSdk {
  private bitrielWallet: BitrielWalletAPI | ZeroDevWalletAPI | SimpleAccountAPI;
  ...
  constructor(walletProvider: WalletProviderLike, optionsLike: SdkOptions) {
    ...
    this.factoryUsed = optionsLike.factoryWallet ?? Factory.BITRIEL;
    ...
    if (this.factoryUsed === Factory.ZERO_DEV) {
      this.bitrielWallet = new ZeroDevWalletAPI({
        provider,
      ...
      this.bitrielWallet = new SimpleAccountAPI({
        provider,
        walletProvider: walletConnectProvider ?? walletProvider,
        optionsLike,
        entryPointAddress,
      ...
      this.bitrielWallet = new BitrielWalletAPI({
        provider,
        walletProvider: walletConnectProvider ?? walletProvider,
        optionsLike,
        entryPointAddress,
  ...
  get state(): StateService {
    return this.bitrielWallet.services.stateService;
  }

  get state$(): BehaviorSubject<State> {
    return this.bitrielWallet.services.stateService.state$;
  }

  get supportedNetworks(): Network[] {
    return this.bitrielWallet.services.networkService.supportedNetworks;
  }
   ...
   */
  destroy(): void {
    this.bitrielWallet.context.destroy();
  }
  ...
  async signMessage(dto: SignMessageDto): Promise<string> {
    ...
    await this.bitrielWallet.require({
      network: false,
    });

    return this.bitrielWallet.services.walletService.signMessage(message);
  }

  async getCounterFactualAddress(): Promise<string> {
    return this.bitrielWallet.getCounterFactualAddress();
  }
  ...
  async estimate(
    ...
    if (paymasterDetails?.url) {
      const paymasterAPI = new VerifyingPaymasterAPI(
        ...
        this.bitrielWallet.entryPointAddress,
        paymasterDetails.context ?? {},
      );
      this.bitrielWallet.setPaymasterApi(paymasterAPI);
    } else this.bitrielWallet.setPaymasterApi(null);
    ...
    const partialtx = await this.bitrielWallet.createUnsignedUserOp(
      {
        ...tx,
  ...
  async getGasFee() {
    ...
    return getGasFee(this.bitrielWallet.provider as providers.JsonRpcProvider);
  }

  async send(userOp: UserOperationStruct) {
    const signedUserOp = await this.bitrielWallet.signUserOp(userOp);
    return this.bundler.sendUserOpToBundler(signedUserOp);
  }

  async getNativeBalance() {
    if (!this.bitrielWallet.accountAddress) {
      await this.getCounterFactualAddress();
    }
    const balance = await this.bitrielWallet.provider.getBalance(this.bitrielWallet.accountAddress);
  ...
  async getUserOpHash(userOp: UserOperationStruct) {
    return this.bitrielWallet.getUserOpHash(userOp);
  }
  ...
  async getAccountContract() {
    return this.bitrielWallet._getAccountContract();
  }
